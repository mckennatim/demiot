...
void createConfigServer(int webtype){
  server.on("/", []() {
      ...
      content += "<meta name='viewport' content='width=device-width, minimum-scale=1, maximum-scale=1'></head>";
...
void loop() {
  ...
  if (now - lastMsg > 1000 && autom) {
    ...
    temp1 = (int)temp;
    if (oldtemp1 != temp1 || oldtemp2 != temp2 || oldLed != digitalRead(ALED)) {
      ...
      sprintf(astr, "{\"temp1\":%d, \"temp2\":%d, \"hoa\":%d, \"heat\":%d }", temp1, temp2, hoa, aled);
      Serial.print(astr);
